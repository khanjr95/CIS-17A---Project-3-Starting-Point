...
void Guild::AddAdvenClass(std::shared_ptr<AdvenClass> newAdven)
{
	_advClass.push_back(newAdven);
}
...
std::string Guild::AttackWithAllAdventurers()
{
	...
	/*if (_advClass.size() > 0) {
		...
		if (_advClass.size() > 0) {
			for (auto paladin : _advClass) {
				output += paladin->Attack() + "\n";
			}
		}
		if (_advClass.size() > 0) {
			for (auto warrior : _advClass) {
				output += warrior->Attack() + "\n";
			}
